...
Figure A1 (see appendix) provides a three-panel overview of the complete system.
Panel A shows the end-to-end pipeline from raw EEG and game signals
to protocol action, with the reward feedback loop. Panel B formalises
the MDP: state vector enters the agent, which selects Hold/Raise,
observes the environment, and receives r(t) = ΔProtocolValue.
Panel C compares both approaches: supervised achieves 67.6–76.1%
DirAcc without exploration; RL requires online exploration to
overcome the 94:6 action imbalance.
...
| A1 | `system_architecture.png` | Appendix: three-panel system architecture (pipeline · MDP cycle · approach comparison) |
...
![System architecture overview. Panel A: end-to-end pipeline from EEG and game signals to protocol action with ΔPV reward feedback. Panel B: MDP reinforcement learning cycle (State → Agent → Environment → reward). Panel C: supervised vs. offline RL comparison with empirical results.](../outputs/figures/system_architecture.png)
